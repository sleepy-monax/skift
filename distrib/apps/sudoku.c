...
int sudoku_check(char sudoku[9][9], int line, int column, char n)
{
	...
	while (i < 3)
	{
		...
		while (j < 3)
		{
			if (sudoku[i + line_start][j + column_start] == n) return 0;
			j++;
		}
		i++;
	}
...
int main(int argc, char **argv)
{
	...
	if (argc == 10 && sudoku_parse(sudoku, ++argv) && sudoku_solve(sudoku)) return 0;
	write(1, "Error\n" , 7);
